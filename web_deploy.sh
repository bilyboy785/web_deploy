...
function init_server {
    ...
    DEBIAN_FRONTEND=noninteractive apt update > /dev/null 2>&1 && DEBIAN_FRONTEND=noninteractive apt upgrade -y > /dev/null 2>&1
    echo "# Installing base packages"
    DEBIAN_FRONTEND=noninteractive apt install -y git zsh curl wget htop python3 bat ripgrep exa fail2ban python3-venv python3-pip proftpd mariadb-client mariadb-server docker.io redis-server > /dev/null 2>&1
