...
case $1 in
    ...
    healthcheck)
        ...
        do
            ...
            echo "## $SITE_NAME"
            case $SITE_NAME in
                dtses.bldwebagency.fr|jerome-tavernier.com|dev.bldwebagency.fr|www.dansetousstyles.quebec|generationsjardins.com)
                    continue
                    ;;
                frenchmac.com)
                    CRON_MIN="15"
            ...
            if [[ -z ${HC_PING_URL} ]]; then
                ...
                echo -e "MAILTO=\"\"\n*/${CRON_MIN} * * * *  RID=\`uuidgen\` && curl -fsS -m 10 --retry 5 -o /dev/null ${HC_PING_URL_RESULT}/start?rid=\$RID && TMP_VAR=$(pwgen 10 -1) && /usr/local/bin/wp --path=${WEB_ROOT} cron event run --due-now > /tmp/\$TMP_VAR.log 2>&1 && curl -fsS -m 10 --retry 5 --data-binary @/tmp/\$TMP_VAR.log ${HC_PING_URL_RESULT}?rid=\$RID" | crontab -u ${OWNER} -
            else
                echo -e "MAILTO=\"\"\n*/${CRON_MIN} * * * *  RID=\`uuidgen\` && curl -fsS -m 10 --retry 5 -o /dev/null ${HC_PING_URL}/start?rid=\$RID && TMP_VAR=$(pwgen 10 -1) && /usr/local/bin/wp --path=${WEB_ROOT} cron event run --due-now > /tmp/\$TMP_VAR.log 2>&1 && curl -fsS -m 10 --retry 5 --data-binary @/tmp/\$TMP_VAR.log ${HC_PING_URL}?rid=\$RID" | crontab -u ${OWNER} -
            fi
