...
case $1 in
    ...
    cloudflarebanip)
        ...
        case $ACTION in
            ban)
                BAN_BLDWEBAGENCY=$(/usr/bin/curl -sX POST "https://api.cloudflare.com/client/v4/accounts/c05ed148df8541c4a08304f3bf28ac26/rules/lists/03d7549fa1f2473e87e0bd412d4c8e9b/items" \
                ...
                BAN_DTS=$(/usr/bin/curl -sX POST "https://api.cloudflare.com/client/v4/accounts/3eda1db40e33ad381b6757dffe5aceb5/rules/lists/ce115bd795264e8da49e181d893b74fa/items" \
                -H "X-Auth-Email: ${CF_EMAIL}" -H "X-Auth-Key: ${CF_APIKEY}" -H "Content-Type: application/json" --data '[{"comment": "From Fail2Ban", "ip":"'${IP}'"}]' | /root/.local/bin/jq -r '.success')
                ...
                ;;
            unban)
                ITEM_ID=$(/usr/bin/curl -sX GET "https://api.cloudflare.com/client/v4/accounts/c05ed148df8541c4a08304f3bf28ac26/rules/lists/03d7549fa1f2473e87e0bd412d4c8e9b/items" \
                ...
                DELETE_BLDWEBAGENCY=$(/usr/bin/curl -sX DELETE "https://api.cloudflare.com/client/v4/accounts/c05ed148df8541c4a08304f3bf28ac26/rules/lists/03d7549fa1f2473e87e0bd412d4c8e9b/items" \
                ...
                ITEM_ID=$(/usr/bin/curl -sX GET "https://api.cloudflare.com/client/v4/accounts/3eda1db40e33ad381b6757dffe5aceb5/rules/lists/ce115bd795264e8da49e181d893b74fa/items" \
                        -H "X-Auth-Email: ${CF_EMAIL}" -H "X-Auth-Key: ${CF_APIKEY}" -H "Content-Type: application/json" | /root/.local/bin/jq '.result[] | select(.ip == "'${IP}'")' | /root/.local/bin/jq -r '.id')
                DELETE_DTS=$(/usr/bin/curl -sX DELETE "https://api.cloudflare.com/client/v4/accounts/3eda1db40e33ad381b6757dffe5aceb5/rules/lists/ce115bd795264e8da49e181d893b74fa/items" \
                        -H "X-Auth-Email: ${CF_EMAIL}" -H "X-Auth-Key: ${CF_APIKEY}" -H "Content-Type: application/json" --data '{"items":[{"id":"'$ITEM_ID'"}]}' | /root/.local/bin/jq -r '.success')
    ...
    healthcheck)
        CRON_MIN="10"
        find /var/www/html -maxdepth 3 -type f -name "wp-config-sample.php" -exec rm -f '{}' \;
        for site in $(find /var/www/html -maxdepth 3 -type f -name "wp-config.php")
        do
            SITE_NAME=$(echo $site | cut -d\/ -f5)
            echo "## $SITE_NAME"
            case $SITE_NAME in
                dtses.bldwebagency.fr|jerome-tavernier.com|dev.bldwebagency.fr|generationsjardins.com)
                    continue
                    ;;
                frenchmac.com)
                    CRON_MIN="15"
            ...
            if [[ -z ${HC_PING_URL} ]]; then
                ...
                echo -e "MAILTO=\"\"\n*/${CRON_MIN} * * * *  RID=\`uuidgen\` && curl -fsS -m 10 --retry 5 -o /dev/null ${HC_PING_URL_RESULT}/start?rid=\$RID && /usr/local/bin/wp --path=${WEB_ROOT} cron event run --due-now && curl -fsS -m 10 --retry 5 -o /dev/null ${HC_PING_URL_RESULT}?rid=\$RID" | crontab -u ${OWNER} -
            else
                echo -e "MAILTO=\"\"\n*/${CRON_MIN} * * * *  RID=\`uuidgen\` && curl -fsS -m 10 --retry 5 -o /dev/null ${HC_PING_URL}/start?rid=\$RID && /usr/local/bin/wp --path=${WEB_ROOT} cron event run --due-now && curl -fsS -m 10 --retry 5 -o /dev/null ${HC_PING_URL}?rid=\$RID" | crontab -u ${OWNER} -
            fi
